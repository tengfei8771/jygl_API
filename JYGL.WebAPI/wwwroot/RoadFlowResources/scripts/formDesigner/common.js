...
function compileForm(attrJson, eventsJSON, subtableJSON, html) {
    ...
    formHtml += '\tstring formData = showArchive ? new FlowArchive().GetArchiveData(Request.Querys("archiveid").ToGuid()) : new Form().GetFormData("' + attrJson.dbConn + '", "' + attrJson.dbTable + '", "' + attrJson.dbTablePrimaryKey + '", instanceId, stepId, flowId, "[' + compieform_formats.join(',') + ']", out fieldStatusJSON);\r\n';
    formHtml += '}\r\n';
    ...
    formHtml += '\tvar defaultValuesJSON = @Html.Raw(new Form().GetDefaultValuesJSON("[' + compieform_defaultvalues.join(",") + ']", fieldStatusJSON));\r\n';
...
function setSelectHtml($element, eventsJSON, $html) {
    ...
    if ("0" == datasource) {//数据字典
        ...
        var valueField = "Dictionary.ValueField.Id";
        ...
        $element.text(("1" == hasempty ? '<option value="">' + emptytitle + '</option>' : '')
            + '@Html.Raw(new Dictionary().GetOptionsByID' +
        ...
        $element.text(("1" == hasempty ? '<option value="">' + emptytitle + '</option>' : '')
            + '@Html.Raw(new Form().GetOptionsBySQL("' + ds_sql_dbconn + '", "' + ds_sql_value + '", ""))');
    } else if ("3" == datasource) {//URL
        var ds_url_address = RoadUI.Core.unescape($element.attr("data-ds_url_address"));
        $element.text(("1" == hasempty ? '<option value="">' + emptytitle + '</option>' : '')
            + '@Html.Raw(new Form().GetOptionsByUrl("' + ds_url_address + '"))');
    }
...
function setRadioHtml($element, eventsJSON) {
    ...
    $element.after('@Html.Raw(new Form().GetRadioOrCheckboxHtml(' + datasource + ', "' + ds_sql_dbconn + '", "' + ds_dict_value + '", "'
...
function setCheckboxHtml($element, eventsJSON) {
    ...
    $element.after('@Html.Raw(new Form().GetRadioOrCheckboxHtml(' + datasource + ', "' + ds_sql_dbconn + '", "' + ds_dict_value + '", "' + ds_dict_valuefield.toLowerCase()
...
function setSignature($element, eventsJSON) {
    ...
    $element.attr("value", "签章").removeAttr("style", "").attr("class", "mybutton").attr("data-id", id)
        .attr("id", id + "_text").attr("name", id + "_text").attr("data-src", "@Url.Content(\"~\" + new User().GetSignSrc())");
...
function setDataTable($element, eventsJSON, connId) {
    ...
    $element.after('@Html.Raw(new Form().GetDataTableHtml("' + width + '", "' + height + '", "' + datasource + '", "' + datasourcetext + '", "' + connId + '", formData))');
    $element.remove();
}
...
function setSubtableHtml($element, eventsJSON, subtableJSON, connId) {
    ...
    if (subJSON) {
        ...
        if ("0" == editmodel) {
            ...
            subHTML += '\t\tSystem.Data.DataTable ' + subtableMapListName + ' = new DbConnection().GetDataTable(new DbConnection().Get("' + connId + '".ToGuid()), "' + secondtable + '", "' + secondtablerelationfield + '", instanceId, "' + sortstring + '");\r\n';
            ...
            for (i = 0; i < subJSON.colnums.length; i++) {
                ...
                switch (editmode) {
                    ...
                    case "select":
                        ...
                        switch (select_ds) {
                            case "select_dsdict":
                                select_ds_ValueField = 'Dictionary.ValueField.Id';
                                switch (select_ds_dict_valuefield) {
                                    case "id":
                                        select_ds_ValueField = 'Dictionary.ValueField.Id';
                                        break;
                                    case "code":
                                        select_ds_ValueField = 'Dictionary.ValueField.Code';
                                        break;
                                    case "value":
                                        select_ds_ValueField = 'Dictionary.ValueField.Value';
                                        break;
                                    case "title":
                                        select_ds_ValueField = 'Dictionary.ValueField.Title';
                                        break;
                                    case "note":
                                        select_ds_ValueField = 'Dictionary.ValueField.Note';
                                        break;
                                    case "other":
                                        select_ds_ValueField = 'Dictionary.ValueField.Other';
                                        break;
                                }
                            ...
                            case "select_dssql":
                                select_options = ("1" == select_hasempty ? '<option value=""></option>' : '')
                                    + '@Html.Raw(new Form().GetOptionsBySQL("' + connId + '", "' + select_ds_sql + '", value_' + field + '))';
                                break;
                    ...
                    case "checkbox":
                        ...
                        tdHTML += '@Html.Raw(new Form().GetRadioOrCheckboxHtml(' + checkbox_source + ', "' + connId + '", "' + checkbox_ds_dict + '", "' + checkbox_ds_dict_valuefield
                    ...
                    case "radio":
                        ...
                        tdHTML += '@Html.Raw(new Form().GetRadioOrCheckboxHtml(' + radio_source + ', "' + connId + '", "' + radio_ds_dict + '", "' + radio_ds_dict_valuefield
